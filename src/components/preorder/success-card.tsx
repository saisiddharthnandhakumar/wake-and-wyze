...
export function SuccessCard({ id, orderNumber, totalPaise, flavor, quantity }: SuccessCardProps) {
  useEffect(() => {
  ...
  }, [totalPaise, orderNumber, flavor, quantity]);

  const flavorName = FLAVORS.find((f) => f.id === flavor)?.name ?? flavor;

  const details = [
    { label: "Order Number", value: orderNumber },
    { label: "Flavor", value: flavorName },
    { label: "Quantity", value: `${quantity} bag${quantity > 1 ? "s" : ""}` },
  ...
  return (
    <Card className="p-8 lg:p-10 text-center">
      ...
      <dl className="mx-auto mt-8 max-w-sm divide-y divide-border-light rounded-2xl border border-border bg-surface text-left">
        {details.map((detail) => (
          <div key={detail.label} className="flex items-center justify-between gap-4 px-5 py-3.5">
